...
class Url extends \Magento\Catalog\Model\ResourceModel\Url
{
    ...
    {
        return $this->filter->translitUrl($product->getName()) . '-' . $product->getId();
    }
            ...
            foreach (['name', 'url_key', 'url_path'] as $attributeCode) {
                ...
                foreach ($attributes as $productId => $attributeValue) {
                    if ($attributeCode == 'url_key' && empty($attributeValue)) {
                        $products[$productId]->setData($attributeCode, $this->formatUrlKey($products[$productId]));
                        $this->saveProductAttribute($products[$productId], $attributeCode);
                    } else {
     ...
     * @return \Magebuzz\Sinchimport\Model\ResourceModel\Product
     */
    public function saveProductAttribute(\Magento\Catalog\Model\Product $product, $attributeCode)
    {
        $connection = $this->getConnection();
