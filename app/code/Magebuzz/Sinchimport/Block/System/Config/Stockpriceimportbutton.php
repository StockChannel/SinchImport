...
class Stockpriceimportbutton extends \Magento\Config\Block\System\Config\Form\Field
{
    ...
    protected function _getElementHtml(AbstractElement $element)
    {
        ...
        $startImportButtonHtml = $this->getLayout()->createBlock(
            ...
            ['label' => 'Force Stock & Prices Import Now', 'id' => 'mb-sinch-stock-price-import-button', 'class' => 'mb-start-button', 'style' => 'margin-top:30px']
        )->toHtml();
        ...
        } elseif (!$sinchModel->isFullImportHaveBeenRun()){
            $html .="Full import have never finished with success";
        }else{
            $html .= $startImportButtonHtml;
        }

        $lastImportData = $sinchModel->getDataOfLatestImport();
        $lastImportStatus = $lastImportData['global_status_import'];

        if ($lastImportStatus == 'Failed') {
            $html .= '<div id="sinchimport_stock_price_current_status_message" name="sinchimport_stock_price_current_status_message" style="display:true"><br><br><hr/><p class="sinch-error">The import has failed. Please ensure that you are using the correct settings. Last step was "' . $lastImportData['detail_status_import'] . '"<br> Error reporting : "' . $lastImportData['error_report_message'] . '"</p></div>';
        } elseif ($lastImportStatus == 'Successful') {
            $html .= '<div id="sinchimport_stock_price_current_status_message" name="sinchimport_stock_price_current_status_message" style="display:true"><br><br><hr/><p class="sinch-success">' . $lastImportData['number_of_products'] . ' products imported succesfully!</p></div>';
    ...
    {
        $runningIcon = $this->_urlBuilder->getBaseUrl(['_type' => UrlInterface::URL_TYPE_MEDIA])
            . 'magebuzz/sinchimport/ajax_running.gif';
