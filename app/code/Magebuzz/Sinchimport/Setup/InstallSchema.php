...
class InstallSchema implements InstallSchemaInterface
{
    ...
    public function install(SchemaSetupInterface $setup, ModuleContextInterface $context)
    {
        ...
        // v0.1.0
        $installer->run("DROP TABLE IF EXISTS {$installer->getTable('sinch_features_list')}");

        $installer->run("CREATE TABLE {$installer->getTable('sinch_features_list')}(
        ...
        ");
        $installer->run("
            CREATE TABLE " . $installer->getTable('sinch_import_status_statistic') . "(
        ...
        $installer->run("
            INSERT " . $installer->getTable('sinch_sinchcheck') . "(caption, descr, check_code,
                    ...
                    error_msg, fix_msg)
                VALUE('Missing Procedure', 'checking absense of procedure " . $installer->getTable('sinch_filter_products') . ".sql store procedue and showing hot to add it', 'routine',
                    '" . $installer->getTable('sinch_filter_products') . "', '',
                    'You are missing the MySQL stored procedure " . $installer->getTable('sinch_filter_products') . ".sql', 'You can recreate it by running the script found in [shop dir]/app/code/local/Bintime/Sinchimport/sql/ in PhpMyAdmin');
        ");
        ...
        // v0.1.9 - 0.2.0
        $installer->run("
            CREATE TABLE IF NOT EXISTS " . $installer->getTable('sinch_products_mapping') . "(
        ...
        $installer->run("
            CREATE TABLE IF NOT EXISTS " . $installer->getTable('sinch_products') . "(
                store_product_id int(11),
                sinch_product_id int(11),
                product_sku varchar(255),
        ...
        $installer->run("
            CREATE TABLE IF NOT EXISTS " . $installer->getTable('sinch_categories_features') . "(
        ...
        $installer->run("
            CREATE TABLE IF NOT EXISTS " . $installer->getTable('sinch_restricted_values') . "(
        ...
        $installer->run("
            CREATE TABLE IF NOT EXISTS " . $installer->getTable('sinch_product_features') . "(
        ...
        $installer->run("
            CREATE TABLE IF NOT EXISTS " . $installer->getTable('sinch_categories_mapping') . "(
        ...
        $installer->run("
            DROP TABLE IF EXISTS " . $installer->getTable('sinch_distributors_stock_and_price') . ";
        ");

        $installer->run("
            CREATE TABLE " . $installer->getTable('sinch_distributors_stock_and_price') . "(
        ...
        // v3.0.5 - 3.0.6
        $installer->run("
            DROP TABLE IF EXISTS " . $installer->getTable('sinch_product_backup') . ";
        ");
        ...
        $installer->run("
            DROP TABLE IF EXISTS " . $installer->getTable('sinch_category_backup') . ";
        ");
        ...
        $installer->run("DROP PROCEDURE IF EXISTS " . $installer->getTable('sinch_filter_products'));
        $installer->run("DROP FUNCTION IF EXISTS " . $installer->getTable('sinch_calc_price'));
