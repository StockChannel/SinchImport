...
class IndexManagement {
    ...
    public function reindexProductUrls(): void
    {
        ...
        $catalog_product_entity =  $this->resourceConn->getTableName('catalog_product_entity');
        $catalog_product_entity_varchar = $this->resourceConn->getTableName('catalog_product_entity_varchar');

        $conn = $this->resourceConn->getConnection();
        $conn->query("DELETE ur FROM $url_rewrite ur 
                INNER JOIN $catalog_product_entity cpe ON ur.entity_id = cpe.entity_id  
                WHERE ur.is_autogenerated = 1 AND ur.entity_type = 'product' AND cpe.sinch_product_id IS NOT NULL"
        );
        $conn->query(
            "UPDATE $catalog_product_entity_varchar cpev 
            INNER JOIN $catalog_product_entity cpe ON cpev.entity_id = cpe.entity_id 
            SET value = '' 
            WHERE attribute_id = :attrId AND cpe.sinch_product_id IS NOT NULL",
