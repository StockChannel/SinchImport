<?php
	...
	class ProductTypeFrequency extends AbstractImportSection {
		...
		public function populateSinchProductTypes()
		{
			...
			$parse_file = $this->downloadHelper->getSavePath ( Download::FILE_PRODUCT_TYPES );
			$product_types_temp = $this->getTableName ( 'sinch_product_types_temp' );
			
			$this->log("DISABLE FOREIGN KEYS CONTRAINTS");
			...
			$this->conn->query(
                ...
                sinch_product_type_id int(11) PRIMARY KEY,
		...
		{
			$parse_file = $this->downloadHelper->getSavePath ( Download::FILE_PRODUCT_FREQUENCIES );
			...
			$this->conn->query(
                ...
                sinch_product_frequency_id int(11) PRIMARY KEY,
		...
		{
			$parse_file = $this->downloadHelper->getSavePath ( Download::FILE_PRODUCT_TYPE_FREQUENCY );
			...
			$this->conn->query(
                ...
                FOREIGN KEY (`sinch_product_id`) REFERENCES `{$this->sinchProductsTable}` (`sinch_product_id`),
                FOREIGN KEY (`sinch_product_type_id`) REFERENCES `{$this->sinchProductTypesTable}` (`sinch_product_type_id`),
                FOREIGN KEY (`sinch_product_frequency_id`) REFERENCES `{$this->sinchProductFrequenciesTable}` (`sinch_product_frequency_id`)
            )ENGINE=InnoDB DEFAULT CHARSET=utf8 DEFAULT COLLATE=utf8_general_ci"
			);
			$this->endTimingStep();
